...
Pipes string_filter(std::string& str) noexcept {
  if (str.size() % 2 == 1) {
    return Pipes::FIRST;
  }
...
void input_handler(int pipe1_write, int pipe2_write) noexcept {
  ...
  while (true) {
    ...
    Pipes filter_code = string_filter(buf);
...
void first_process_handler(int pipe1_read, int file1_desc) {
  ...
  int status = execl("/home/b_eli/Desktop/MAI_OS/lab1/build/lab1_main_first", "/home/b_eli/Desktop/MAI_OS/lab1/build/lab1_main_first", NULL);
  if (status < 0) {
    throw std::runtime_error("error");
  }
}
...
void second_process_handler(int pipe2_read, int file2_desc) {
  ...
  int status = execl("/home/b_eli/Desktop/MAI_OS/lab1/build/lab1_main_second", "/home/b_eli/Desktop/MAI_OS/lab1/build/lab1_main_second", NULL);
  if (status < 0) {
    throw std::runtime_error("error");
  }  
}
